	...
	try {
		...
		for (let i = 0; i < playbookTableColumns; i++) {
			...
			Array.from(playbookFullTable.rows).forEach(function () {
				if (rowIndex > 0) {
					try {
						var classList = Array.from(
							$(playbookFullTable.rows[rowIndex].cells[i])[0].firstElementChild.classList
						);
			...
			$("#categoryFilter")
				...
				.each(function (index, elem) {
					...
					if (rawClasses[i].includes(category)) {
						let difference = rawClasses[i].filter(function (x) {
							return ![category].includes(x);
						});
		...
		function reRenderPlaybook() {
			...
			for (let i = 0; i < playbookTableColumns; i++) {
				...
				Array.from(playbookFullTable.rows).forEach(function () {
					if (rowIndex > 0) {
						...
						var categoryIntersection = categoryFilterSelections.filter(function (element) {
							return classList.includes(element);
						});
						var toolingIntersection = toolingFilterSelections.filter(function (element) {
							return classList.includes(element);
						});
						...
						if (categoryFilterSelections.length >= 1 && categoryIntersection.length >= 1) {
							...
							if (
								toolingFilterSelections.length >= 1 &&
								toolingIntersection.length >= 1 &&
								categoryIntersection.length >= 1
							) {
						...
						// if there is no selection in the category filter and a selection in the tooling filter and the class of the cell matches
							...
							categoryFilterSelections.length == 0 &&
							toolingFilterSelections.length >= 1 &&
							toolingIntersection.length >= 1
						) {
							columnFilteredEntries.push(cellHTMLValue);
							...
							categoryFilterSelections.length == 0 &&
							toolingFilterSelections.length == 0
						) {
							columnFilteredEntries.push(cellHTMLValue);
							columnFilteredEntriesMap.push(classList);
						}
			...
			for (var i = 0; i < columnFilteredEntriesMap.length; i++) {
				if (
					columnFilteredEntriesMapFlat.indexOf(columnFilteredEntriesMap[i]) === -1 &&
					columnFilteredEntriesMap[i] !== ""
				) {
				...
				.each(function (index, elem) {
					...
					// if there are selections in the category filter
						...
						// if there are selections in the tooling filter and the tooling filter includes the value, green
							...
							toolingFilterSelections.length >= 1 &&
							toolingFilterSelections.includes($(elem).val())
						) {
							$(elem).removeClass("dark-gray");
							$(elem).removeClass("light-gray");
							$(elem).addClass("green");
				...
				.each(function (index, elem) {
					...
					// if there are selections in the tooling filter
						...
						// if there are selections in the category filter and the category filter includes the value, green
							...
							categoryFilterSelections.length >= 1 &&
							categoryFilterSelections.includes($(elem).val())
						) {
							$(elem).removeClass("dark-gray");
							$(elem).removeClass("light-gray");
							$(elem).addClass("green");
	...
	$(".navigation-list-child-list").each(function () {
		$($(this)[0].childNodes).each(function () {
			...
			try {
				...
				$(this)[0].firstChild.innerHTML = $(this)[0].firstChild.innerHTML.replace(
					$(this)[0].firstChild.innerText,
					$(this)[0].firstChild.innerText + " (" + itemCount + ")"
				);
			} catch (e) {}
		});
	});
	...
	$(".navigation-list-child-list").each(function () {
		try {
			...
			if (innerText != "System Planning") {
				...
				childrenRemove = context.find(
					".navigation-list-grandchild-list > .navigation-list-item"
				).length;
				count = children.length - childrenRemove;
			}
			$(this)[0].parentElement.firstChild.innerHTML = innerHTML.replace(
				innerText,
				innerText.replace(" and ", " & ") + " (" + count + ")"
			);
			a.data("cnt", count);
		} catch (e) {}
	});
