...
If the **App Metadata Analyzer** has not yet been imported into the site, navigate to the [Where to get it](../../tooling/app_metadata_analyzer.md#where-to-get-it) section of the [App Metadata Analyzer](../../tooling/app_metadata_analyzer.md) for instructions on where to find the application.
...
If the **App Metadata Analyzer** is not up-to-date, please refer to the [App Metadata Analyzer](../../tooling/app_metadata_analyzer.md#documentation) for configuration details and troubleshooting steps.
...
- **Prod**
  - Use for optimization. Refer to the [Optimization](#optimization) section below.
  - Identify candidates for load balancing. Refer to the [load balancing section](../../system_planning/review_update_capacity_plan/applications.md#app-metadata-analyzer-1) of the [Review/Update Capacity Plan](../../system_planning/review_update_capacity_plan.md) article.
...
Refer to the _App Memory Footprint (MB)_ table. In this example, there is an application that takes up ~63 GB in RAM.

![ama_4.png](images/ama_4.png)

Prioritize applications that are both large _and_ widely used. Refer to [Analyze App Adoption](analyze_app_adoption.md) for how to view the user activity on an application, and review each large application from this exercise there.
...
Refer to the [load balancing section](../../system_planning/review_update_capacity_plan/applications.md#app-metadata-analyzer-1) of the [Review/Update Capacity Plan](../../system_planning/review_update_capacity_plan.md) article, where this process is explained.

For more detail on load balancing of assets and instructions on how to do so, refer to the [Segregation of Larger Qlik Apps](review_pinning_load_balancing.md#scenario-2---segregation-of-larger-qlik-apps) of the [Review Pinning/Load Balancing](review_pinning_load_balancing.md) article.
